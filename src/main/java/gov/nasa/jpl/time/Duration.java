...
public class Duration implements Comparable<Duration>, ConvertableFromString {
    ...
    public final static long ONE_MILLISECOND                        = ONE_SECOND/1000L;
    public final static long ONE_MICROSECOND                        = ONE_SECOND/1000000L;

    public final static Duration ZERO_DURATION          = new Duration(0);
    public final static Duration MICROSECOND_DURATION   = new Duration(ONE_MICROSECOND);
    public final static Duration SECOND_DURATION        = new Duration(ONE_SECOND);
     ...
     * IMPORTANT: only intended for fast deserialization and IO assuming one backing representation of tics
     * and is not portable between programs due to useSpiceForMath and/or backing changes in the future
     */
    public static Duration fromTics(long tics){
        return new Duration(tics);
    }
    ...
    public static Duration fromMilliseconds(double ms) {
        return MICROSECOND_DURATION.multiply(1000.0*ms);
    }
    ...
    public static Duration fromMilliseconds(long ms) {
        return new Duration(ONE_MILLISECOND*ms);
    }
     ...
     * IMPORTANT: assumes that find() has already been called, the matcher is queued with the groups to parse,
     * and higher-level nice error messages are available to tell the user why the call failed/will fail.
     * This is for performance reasons - if the calling program is doing its own input validation, you don't want to do it twice
     */
    public static Duration fromMatcher(Matcher parsedDuration){
        return new Duration(computeTicsFromMatcher(parsedDuration));
    }

    private static long computeTicsFromMatcher(Matcher durationMatcher){
        long toReturn;
        ...
        toReturn = days * ONE_DAY + hours * ONE_HOUR + minutes * ONE_MINUTE + Math.round(seconds * ONE_SECOND);
        toReturn = toReturn * negativeMultiplier;
        return toReturn;
    }
     ...
     * @param modulus
     * @return The calling duration mod the input parameter
     */
    public Duration mod(Duration modulus){
        return new Duration(tics % modulus.tics);
    }
    ...
    public void valueOf(String s) {
        ...
        if (durationMatcher.find()) {
            tics = computeTicsFromMatcher(durationMatcher);
        }
        else {
